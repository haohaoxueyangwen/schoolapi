...
def _parse_malformed_named_tool_payload(raw, tool_name=None):
    """Recover model-emitted JSON with unescaped quotes inside string values.

    Handles cases like:
      {"name": "Bash", "arguments": {"command": "echo "EXIT: $?""}}
    where inner quotes break standard JSON parsing.
    """
    ...
    # Extract the arguments object using regex: find "arguments": { ... last }
    args_match = re.search(r'"arguments"\s*:\s*\{', normalized)
    if not args_match:
        return None

    args_start = args_match.end() - 1  # include the opening {
    args_end = normalized.rfind('}')
    if args_end <= args_start:
        return None

    args_body = normalized[args_start:args_end + 1]
    ...
    # Fallback: extract key-value pairs where values may contain unescaped quotes
    # For each key, grab everything between the opening quote and the last
    # quote before the next key or closing brace.
    key_positions = list(re.finditer(
        r'"([A-Za-z_][A-Za-z0-9_]*)"\s*:\s*"', args_body
    ))
    parsed_args = {}
    for idx, km in enumerate(key_positions):
        key = km.group(1)
        value_start = km.end()  # right after the opening " of the value
        if idx + 1 < len(key_positions):
            # Find the last quote before the next key's opening "
            next_key_start = key_positions[idx + 1].start()
            chunk = args_body[value_start:next_key_start]
            last_quote = chunk.rfind('"')
            if last_quote >= 0:
                raw_value = chunk[:last_quote]
            else:
                raw_value = chunk.rstrip('",} ')
        else:
            # Last key: find the last quote before closing }
            chunk = args_body[value_start:]
            last_quote = chunk.rfind('"')
            if last_quote >= 0:
                raw_value = chunk[:last_quote]
            else:
