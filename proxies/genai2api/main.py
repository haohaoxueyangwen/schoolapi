...
if not token_input:
    token_file = os.environ.get("GENAI_TOKEN_FILE", os.path.expanduser("~/.claude/genai-token.txt"))
    try:
...
if not token_input:
    parser.error("token is required via --token, GENAI_TOKEN, or GENAI_TOKEN_FILE")
...
if __name__ == '__main__':
    ...
    logger.info("Starting GenAI proxy on %s:%d", config.host, config.port)
