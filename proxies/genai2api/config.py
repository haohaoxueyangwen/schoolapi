...
class ModelRegistry:
    ...
    def get_models(self, token: str, *, _config=None) -> dict[str, ModelInfo]:
        if not self._models or (time.time() - self._last_fetched > self._cache_ttl):
            try:
                self.fetch(token)
            except TokenExpiredError:
                if _config and _config.token_manager.mode == "credential":
                    logger.info("Token expired during model fetch, refreshing via CAS...")
                    new_token = _config.token_manager.force_refresh()
                    if new_token:
                        self.fetch(new_token)
                    else:
                        raise
                else:
                    raise
    ...
    def get_root_ai_type(self, model: str, token: str, *, _config=None) -> str:
        models = self.get_models(token, _config=_config)
...
def build_genai_headers(token: str, extra_headers: dict | None = None) -> dict:
    headers = {
