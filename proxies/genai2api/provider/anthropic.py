...
from context_compress import check_context_size
from genai_types import new_chat_group_id
from tools.parsing import extract_tool_calls, _tag_prefix_len
from tools.prompts import inject_tool_prompt

GENAI_MODEL_FILE = Path.home() / ".claude" / "genai-model.txt"
ANTHROPIC_MODEL_PREFIXES = ("claude-", "anthropic-")

# Token expiration keywords from GenAI backend business errors
_TOKEN_EXPIRED_KEYWORDS = ("Token失效", "token失效", "token expired", "请重新登录", "未登录")


def _is_token_expired_error(err_msg: str) -> bool:
    """Check if an error message indicates token expiration."""
    return any(kw in err_msg for kw in _TOKEN_EXPIRED_KEYWORDS)


def _resolve_model(requested_model: str, token: str = "", config: Any = None) -> str:
    """Map Anthropic model names to the real GenAI model from genai-model.txt.

    Three-way resolution:
    1. Anthropic-prefixed (claude-*, anthropic-*) → map to genai-model.txt
    2. Known GenAI model (verified against GenAI model list) → pass through
    3. Unknown / unrecognized → fall back to genai-model.txt
    """
    # Anthropic model names → always map to genai-model.txt
    if requested_model.startswith(ANTHROPIC_MODEL_PREFIXES):
        try:
            real = GENAI_MODEL_FILE.read_text().strip()
            if real:
                return real
    ...
    # Known GenAI model → pass through unchanged
    if token:
        try:
            valid_models = model_registry.get_models(token, _config=config)
            if requested_model in valid_models:
                return requested_model
        except Exception:
            pass

    # Unknown model (e.g. stale deepseek-v4-pro from a previous profile) →
    # fall back to genai-model.txt
    try:
        real = GENAI_MODEL_FILE.read_text().strip()
        if real:
            return real
...
def anthropic_messages_to_genai_format(body: Dict[str, Any], token: str, config: Any = None) -> Tuple[str, List[Dict[str, Any]], str]:
    """Convert Anthropic Messages API format to GenAI format.

    Returns:
        Tuple of (system_prompt, messages, model)
    """
    model = _resolve_model(body.get("model", "GPT-5.5"), token, config=config)
...
def stream_genai_as_anthropic(
    ...
    model: resolved model name for GenAI backend (aiType)
    original_model: model name to report back to Claude Code (preserves user-facing display)
    """
    ...
    root_ai_type = model_registry.get_root_ai_type(model, token, _config=config)

    # Extract first user message as chatInfo (align with web behavior)
    chat_info = ""
    prompt_tokens = 0
    for msg in reversed(messages):
        if msg.get("role") == "user":
            ...
            if isinstance(content, str):
                chat_info = content[:200]
                prompt_tokens = max(1, len(chat_info) // 4)
            break

    # Check if context is too large — remind user to /compact
    needs_compact = check_context_size(messages)

    chat_group_id = new_chat_group_id()

    genai_data = {
        "chatInfo": chat_info or "hello",
        "messages": messages,
        "type": "3",
        "stream": True,
    ...
    # Use original model name for Claude Code display, resolved for backend
    display_model = original_model or model
    ...
    compact_reminder_shown = False

    def emit_text(text: str) -> Generator[str, None, None]:
        nonlocal first_token_at, text_block_started, delta_count, compact_reminder_shown
        if not text:
            return
        ...
        if needs_compact and not compact_reminder_shown:
            compact_reminder_shown = True
            reminder = "⚠️ 上下文已超过400k字符，建议输入 /compact 压缩对话以避免超时。\n\n"
    ...
    try:
        ...
        for attempt in range(1, max_retries + 1):
            if attempt > 1:
                logger.info("Retry attempt %d/%d for model=%s", attempt, max_retries, model)
            ...
            try:
                response = requests.post(
                    ...
                    json=genai_data,
                    stream=True,
                    timeout=(30, 600)  # (connect, read) — 10min read timeout for streaming
                )
            except (requests.exceptions.Timeout, requests.exceptions.ConnectionError) as net_err:
                logger.warning("Network error (attempt %d): %s", attempt, net_err)
                if attempt < max_retries:
                    continue
                yield write_sse("error", {
                    "type": "error",
                    "error": {"type": "api_error", "message": f"Network error: {net_err}"},
                })
                return
            ...
            if response.status_code != 200:
                ...
                is_quota = '额度' in err_text or 'quota' in err_text.lower()
                logger.warning("GenAI API error %d (attempt %d): %s", response.status_code, attempt, err_text)
                        ...
                        try:
                            ...
                            if line_str:
                                ...
                                if isinstance(genai_json, dict) and genai_json.get("success") is False:
                                    ...
                                    if '额度' in err_msg:
                                        quota_error_in_stream = True
                                        break
                                    if _is_token_expired_error(err_msg) and attempt < max_retries:
                                        logger.warning("Token expired in stream, will refresh and retry (attempt %d)", attempt)
                                        config.token_manager.force_refresh()
                                        quota_error_in_stream = True  # reuse retry path
                                        break
                                ...
                                if isinstance(genai_json, dict) and genai_json.get("code") in (500, 400, 403, 404):
                                    err_msg = genai_json.get("errMsg") or genai_json.get("message") or "Unknown upstream error"
                                    if '额度' in err_msg:
                                        quota_error_in_stream = True
                                        break
                                    if _is_token_expired_error(err_msg) and attempt < max_retries:
                                        logger.warning("Token expired (code=%s), will refresh and retry (attempt %d)",
                                                       genai_json.get("code"), attempt)
                                        config.token_manager.force_refresh()
                                        quota_error_in_stream = True  # reuse retry path
                                        break
                                    logger.warning("GenAI error (code=%s): %s", genai_json.get("code"), err_msg)
                                    yield write_sse("error", {
                                        "type": "error",
                                        "error": {"type": "api_error", "message": f"Upstream error: {err_msg}"},
                                    })
            ...
            # Handle stream error during iteration
            if stream_error:
                if got_content:
                    logger.warning("Stream interrupted after content (%.1fs): %s",
                                   time.monotonic() - started_at, stream_error)
                    finished = True
                elif attempt < max_retries:
                    logger.info("Stream error before content, retrying (attempt %d): %s", attempt, stream_error)
                    continue
                else:
                    yield write_sse("error", {
                        "type": "error",
                        "error": {"type": "api_error", "message": f"Stream error: {stream_error}"},
                    })
                    return

            # 判断是否需要重试（额度错误或空响应）
            if quota_error_in_stream and attempt < max_retries:
                logger.info("Quota error in stream, retrying (attempt %d)", attempt)
                continue
            if not got_content and not finished and attempt < max_retries:
                logger.info("Empty response from upstream, retrying (attempt %d)", attempt)
                continue
